...
\textcolor{blue}{Perhaps in the modeling discussion below, it will be more clear to discuss the microburst duration separately for non-relativistic and relativistic electron energies.} While we did not find a definitive trend in microburst duration as a function of L, the trend as a function of MLT is sigtnificant. The MLT trend correlates well with a similar trend in chorus rising tone element duration \cite{Teng2017, Shue2019}. However, the chorus wave properties that control the microburst duration are complex. Figure 4 in \citeA{Chen2020} shows a test particle model result and describe what wave properties bound the microburst duration in time-energy space. The upper and lower microburst energy bounds are roughly driven by the initial and final magnetic latitudes of the chorus wave propagation, as expected from the cyclotron resonance condition \cite{Lorentzen2001a}. 
