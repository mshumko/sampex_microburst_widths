...
\end{equation} where $A$, $t_0$, and $\sigma$ are the Gaussian amplitude, center time, and standard deviation; while the $c_0$ and $c_1$ are the background count intercept and slope. The fit was applied over a number of data points determined by the maximum of either: 4x topographic prominence width or 0.5 seconds. \textcolor{red}{A challenge to any roburst automated nonlinear regression algorithm is guessing the initial parameters.} The initial parameter guesses for the Gaussian are provided from the topographic prominence and duration estimate. The linear trend initial parameters were the same: \textcolor{red}{$c_0=50$} and $c_1=0$. The optimal fit parameters were found using Python's \url{scipy.curve_fit()} function. We defined the microburst duration as the full width at half maximum (FWHM) of the microburst peak, defined as
\begin{equation}
